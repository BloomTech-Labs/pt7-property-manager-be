...
router.get("/:id", (req, res) => {
    // get all lease terms
    const { id } = req.params;
    lease_term.findLeaseTermById(id)
        .then(lease_term => {
            if (lease_term) {
                res.status(200).json( { lease_term });
            } else {
        ...
        .catch(err => {
            res.status(500).json({ error: "Failed to get the lease", err: err.message })
        });
});
...
router.put("/lease/:id", (req, res) => {
    ...
    lease_term.updateLeaseTerm(lease_term, id)
        .then(updated => {
            if (updated) {
...
router.delete("/:id", (req, res) => {
    const id = req.params.id;
    lease_term.removeLeaseTerm(id)
        .then(nan => 
            res.status(204).json({ message: `User ${id} has been deleted`})
        )
        .catch(err => 
            res.status(500).json({ error: "Failed to delete lease term", err: err.message})
        );
});
